...
* @param {String} buybackTxSignature - Transaction signature of the buyback transaction
...
try {
  ...
  try {
    ...
    // UPDATED: Include buyback transaction signature
    const burnRecord = {
      id: `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
      timestamp: new Date().toISOString(),
      burnType: 'buyback',
      burnAmount: amount,
      txSignature: signature, // This is the burn transaction signature
      buybackTxSignature: buybackTxSignature, // This is the buyback transaction signature
      ...
      details: {
        ...
        rawAmount: rawAmount,
        buybackTxSignature: buybackTxSignature // Also include in details for redundancy
      }
    };
      ...
      const updatedRewards = rewards.map(r => {
        if (r.id === rewardId) {
          return {
            ...
            buybackTxSignature: buybackTxSignature, // Add buyback tx to reward record too
            status: 'burned',
    ...
    return {
      ...
      burnAddress,
      buybackTxSignature: buybackTxSignature,
