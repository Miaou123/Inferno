...
const initializeStorage = async () => {
  try {
    ...
    logger.info(`Found ${storedMilestones.length} milestones in storage`);
    
    // If no milestones in storage, initialize from burnConfig
    if (storedMilestones.length === 0) {
      logger.info('No milestones found in storage, initializing from burnConfig...');
      
      // Initialize milestones from BURN_SCHEDULE
      BURN_SCHEDULE.forEach(milestone => {
        ...
        fileStorage.saveRecord('milestones', milestoneRecord);
        logger.info(`Created milestone: ${milestone.marketCap} (${milestone.burnAmount.toLocaleString()} tokens)`);
      });
      
      const initializedMilestones = fileStorage.readData(fileStorage.FILES.milestones);
      logger.info(`Successfully initialized ${initializedMilestones.length} milestones in storage`);
    }
...
const checkMilestones = async () => {
  try {
    ...
    // Get all milestones from storage AND config
    const milestones = fileStorage.readData(fileStorage.FILES.milestones);
    logger.info(`Retrieved ${milestones.length} milestones from storage`);
    
    // Log the actual milestones for debugging
    logger.info(`First 3 milestones in storage: ${JSON.stringify(milestones.slice(0, 3))}`);
    
    // If no milestones in storage, use the ones from burnConfig
    if (milestones.length === 0) {
      logger.info('No milestones found in storage, using burn schedule from config');
      
      // Log the burn schedule from config
      logger.info(`Burn schedule from config: ${JSON.stringify(BURN_SCHEDULE.slice(0, 3))}`);
      
      // Initialize milestones in storage from burn config
      BURN_SCHEDULE.forEach(milestone => {
      ...
      const initializedMilestones = fileStorage.readData(fileStorage.FILES.milestones);
      logger.info(`Initialized ${initializedMilestones.length} milestones in storage`);
    }
    
    // Filter for incomplete milestones that have been reached
    const incompleteMilestones = milestones
      .filter(m => {
        const isCompleted = m.completed === true;
        const isReached = currentMarketCap >= m.marketCap;
        
        // Log each milestone evaluation for debugging
        logger.info(`Milestone ${m.marketCap}: completed=${isCompleted}, reached=${isReached} (currentMcap=${currentMarketCap})`);
        
        return !isCompleted && isReached;
      })
    ...
    logger.info(`First pending milestone: ${JSON.stringify(incompleteMilestones[0])}`);
